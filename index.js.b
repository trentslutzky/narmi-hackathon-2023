...
async function getNarmiUsers() {
  ...
  return ( 
    ...
    },
    {
      ...
      is_superuser: false,
      date_joined: '2018-03-26 14:43:59.004000+00:00',
      last_login: '2023-04-27 20:19:50.749892+00:00',
      segment: '',
      metadata: {},
      has_username: true,
      has_password: true,
...
function dateWithinSevenDatysOf(date, prior_date) {
  const prior_date_minus_seven = new Date(new Date().setDate(prior_date.getDate() - 7));
  console.log(date, prior_date, prior_date_minus_seven);
  return prior_date_minus_seven < date && date <= prior_date 
}
...
async function main() {
  ...
  const users_to_notify = users.filter(user => {
    return dateWithinSevenDatysOf(new Date(Date.parse(user.last_login)), priorDateToNotify)
  })
...
async function notify_users(users_list) {
  // TODO: send email to users_list[].email
  return
}
